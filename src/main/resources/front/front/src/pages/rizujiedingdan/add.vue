...
<div :style='{"width":"1000px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"0 auto 20px","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="租借时长" prop="zujieshizhang">
            <el-input v-model="ruleForm.zujieshizhang" 
                placeholder="租借时长" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='zujieshizhang'){
              this.ruleForm.zujieshizhang = obj[o];
              this.ro.zujieshizhang = true;
              continue;
            }
            if(o=='dingjin'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('rizujiedingdan/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('rizujiedingdan/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('rizujiedingdan/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
