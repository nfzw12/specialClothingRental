...
<div :style='{"width":"1000px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"0 auto 20px","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="报修名称" prop="baoxiumingcheng">
            <el-input v-model="ruleForm.baoxiumingcheng" 
                placeholder="报修名称" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="报修价格" prop="baoxiujiage">
            <el-input v-model="ruleForm.baoxiujiage" 
                placeholder="报修价格" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="报修日期" prop="baoxiuriqi" >
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="报修内容" prop="baoxiuneirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='baoxiujiage'){
              this.ruleForm.baoxiujiage = obj[o];
              this.ro.baoxiujiage = true;
              continue;
            }
            if(o=='baoxiuriqi'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('zukebaozhangxiufu/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('zukebaozhangxiufu/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('zukebaozhangxiufu/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
