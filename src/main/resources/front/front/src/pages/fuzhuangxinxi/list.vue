...
<div>
	...
	<div class="list-preview" :style='{"padding":"0 440px","margin":"10px auto","backgroundImage":"url(http://codegen.caihongy.cn/20220913/555a23b143054957981fadfdafb1b189.png)","width":"100%","position":"relative","backgroundPosition":"left bottom","backgroundRepeat":"no-repeat"}'>
		...
		<div class="category-2" :style='{"width":"120px","position":"absolute","left":"280px","flexDirection":"column","display":"flex","height":"auto"}'>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"20px","overflow":"hidden","borderRadius":"20px","flexWrap":"wrap","background":"#c6e8d4","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>服装分类</div>
        <el-select v-model="formSearch.fuzhuangfenlei" placeholder="请选择服装分类" :clearable="true">
          <el-option v-for="(item, index) in fuzhuangfenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('fuzhuangxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#000","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/fuzhuangxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","display":"flex","height":"auto"}'>
			<div :style='{"padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"20px 10px","borderRadius":"10px","background":"#fff","width":"470px","fontSize":"0","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"260px","padding":"10px","margin":"0 0 0 10px","overflow":"hidden","display":"inline-block","height":"auto"}'>
					...
					<div :style='{"overflow":"hidden","color":"#000","width":"100%","lineHeight":"32px","fontSize":"16px","fontWeight":"600","height":"35px"}' class="name ">{{item.fuzhuangyanse}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/fuzhuangfenlei/fuzhuangfenlei').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.fuzhuangmingcheng = '%' + this.indexQueryCondition + '%';
        ...
        if (this.formSearch.fuzhuangfenlei != '') searchWhere.fuzhuangfenlei = this.formSearch.fuzhuangfenlei;
        if (this.curFenlei != '全部') searchWhere.fuzhuangfenlei = this.curFenlei;
        this.$http.get('fuzhuangxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/fuzhuangxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
